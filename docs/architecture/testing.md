...
- **Viz gets its first tested code (T43):** `tests/unit/viz-timeline.test.ts` exercises `viz/timeline.mjs`, the pure reducer behind the Graph tab's time-mode playback; `tests/unit/viz-render-guard.test.ts` is a grep-guard pinning that every full re-render of the graph routes through `requestRender()` — it fails if a second direct call site to `renderGraph()` appears in `viz/app.js`.
